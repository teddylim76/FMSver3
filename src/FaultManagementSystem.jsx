...
// RTDB는 배열을 객체로 저장하므로 다시 배열로 변환하는 헬퍼
...
// ── Photo Upload Component (Firebase Storage) ────────────────────────────────
function PhotoUpload({ photos, setPhotos, label = "사진 업로드" }) {
  const inputRef = useRef();
  const [progress, setProgress] = useState({}); // { [photoId]: 0~100 }

  const handleFile = (e) => {
    const files = Array.from(e.target.files);
    files.forEach((file) => {
      const photoId = genId();
      const path = `photos/${today()}/${photoId}_${file.name}`;
      const sRef = storageRef(storage, path);
      const task = uploadBytesResumable(sRef, file);

      setProgress((prev) => ({ ...prev, [photoId]: 0 }));

      task.on(
        "state_changed",
        (snap) => {
          const pct = Math.round((snap.bytesTransferred / snap.totalBytes) * 100);
          setProgress((prev) => ({ ...prev, [photoId]: pct }));
        },
        (err) => {
          toast("사진 업로드 실패: " + err.message, "error");
          setProgress((prev) => { const n = { ...prev }; delete n[photoId]; return n; });
        },
        async () => {
          const url = await getDownloadURL(task.snapshot.ref);
          setPhotos((prev) => [...prev, { id: photoId, url, name: file.name, storagePath: path }]);
          setProgress((prev) => { const n = { ...prev }; delete n[photoId]; return n; });
      ...
      <div className="photo-upload-area" onClick={() => inputRef.current.click()}>
        ...
        {uploadingCount > 0 ? (
          <div style={{ fontSize: 13, color: "var(--info)", fontWeight: 600 }}>
            업로드 중... {Object.values(progress).map((p, i) => <span key={i}>{p}%</span>).reduce((a, b) => [a, " / ", b], "")}
          </div>
        ) : (
          <div style={{ fontSize: 13, color: "var(--text2)" }}>{label} (클릭하여 파일 선택)</div>
        )}
        <div style={{ fontSize: 11, color: "var(--text2)", marginTop: 4 }}>JPG, PNG, GIF 지원 · Firebase Storage 저장</div>
      </div>
        ...
        <div className="photo-grid">
          {photos.map((p) => (
            <div key={p.id} className="photo-thumb-wrap">
              ...
              <div className="photo-delete" onClick={() => handleDelete(p)}>✕</div>
            </div>
          ))}
        </div>
      )}
...
// PAGE: 로그인 / 회원가입
// ════════════════════════════════════════════════════════════════════════════
function LoginPage({ users, addUser, onLogin }) {
...
// PAGE: 장애신청 (등록 / 수정)
// ════════════════════════════════════════════════════════════════════════════
function FaultForm({ facilities, editFault, setEditFault, setPage, currentUser, saveFault }) {
  ...
  const save = () => {
    ...
    if (isEdit) {
      saveFault({ ...form, id: editFault.id });
      toast("장애 내역이 수정되었습니다.", "success");
      setEditFault(null);
    } else {
      saveFault({ ...form, id: genId() });
...
// PAGE: 장애이력목록
// ════════════════════════════════════════════════════════════════════════════
function FaultList({ faults, deleteFault, faultFilter, setFaultFilter, setEditFault, setPage, currentUser }) {
  const [q, setQ] = useState("");
...
// PAGE: 시설정보 등록
// ════════════════════════════════════════════════════════════════════════════
function FacilityForm({ editFacility, setEditFacility, setPage, saveFacility }) {
  ...
  const save = () => {
    ...
    if (isEdit) {
      saveFacility({ ...form, id: editFacility.id });
      toast("시설 정보가 수정되었습니다.", "success");
      setEditFacility(null);
    } else {
      saveFacility({ ...form, id: genId() });
...
// PAGE: 시설관리현황
// ════════════════════════════════════════════════════════════════════════════
function FacilityList({ facilities, deleteFacility, faults, setEditFacility, setPage }) {
  const [q, setQ] = useState("");
  const [detail, setDetail] = useState(null);
...
// PAGE: 계정관리
// ════════════════════════════════════════════════════════════════════════════
function AccountManagement({ users, addUser, deleteUser, currentUser }) {
...
export default function App() {
  ...
  useEffect(() => {
    const SEED_USERS = [
      ...
      { id: "u3", name: "김현장", email: "kim@its.go.kr", password: "user123", role: "user", dept: "도로유지팀", phone: "010-3333-4444" },
    ];
    const unsubUsers = onValue(dbRef(rtdb, "users"), (snap) => {
      const data = snap.val();
      if (data) setUsers(Object.entries(data).map(([id, v]) => ({ ...v, id })));
      else SEED_USERS.forEach((u) => set(dbRef(rtdb, "users/" + u.id), u));
    });
    const unsubFaults = onValue(dbRef(rtdb, "faults"), (snap) => {
      const data = snap.val();
      if (data) setFaults(Object.entries(data).map(([id, v]) => ({ ...v, id, photos: toArr(v.photos) })));
      else INIT_FAULTS.forEach((f) => set(dbRef(rtdb, "faults/" + f.id), f));
    });
    const unsubFacilities = onValue(dbRef(rtdb, "facilities"), (snap) => {
      const data = snap.val();
      if (data) setFacilities(Object.entries(data).map(([id, v]) => ({ ...v, id, photos: toArr(v.photos) })));
      else INIT_FACILITIES.forEach((f) => set(dbRef(rtdb, "facilities/" + f.id), f));
    });
    return () => { unsubUsers(); unsubFaults(); unsubFacilities(); };
  }, []);

  const saveFault     = (fault)    => set(dbRef(rtdb, "faults/"     + fault.id),    fault);
  const deleteFault   = (id)       => remove(dbRef(rtdb, "faults/"     + id));
  const saveFacility  = (facility) => set(dbRef(rtdb, "facilities/" + facility.id), facility);
  const deleteFacility = (id)      => remove(dbRef(rtdb, "facilities/" + id));
  const addUser       = (user)     => set(dbRef(rtdb, "users/"      + user.id),     user);
  const deleteUser    = (id)       => remove(dbRef(rtdb, "users/"      + id));
  ...
  if (!currentUser) {
    return (
      <>
        <LoginPage users={users} addUser={addUser} onLogin={setCurrentUser} />
        <ToastArea />
      </>
    );
  }
        ...
        <div className="main-area">
          <div className="topbar">
            <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
              <div style={{ fontSize: 13, fontWeight: 800, color: "var(--navy)", letterSpacing: "0.04em" }}>대주씨앤에스(주)</div>
          ...
          <div className="page-content">
            ...
            <div style={{ background: "linear-gradient(135deg, #0B1D3A, #122448)", borderRadius: 10, padding: "10px 18px", marginBottom: 20, display: "flex", alignItems: "center", gap: 12 }}>
              ...
              <div style={{ flex: 1 }}>
                <span style={{ color: "#00C6FF", fontWeight: 700, fontSize: 12 }}>Firebase Realtime Database 연동 완료</span>
                <span style={{ color: "rgba(255,255,255,0.6)", fontSize: 11, marginLeft: 10 }}>
                  Project: fmsver3 | Region: asia-southeast1 | 실시간 데이터 동기화 활성
                </span>
              </div>
            ...
            {page === "fault-form" && (
              <FaultForm facilities={facilities} editFault={editFault} setEditFault={setEditFault} setPage={setPage} currentUser={currentUser} saveFault={saveFault} />
            )}
            {page === "fault-list" && (
              <FaultList faults={faults} deleteFault={deleteFault} faultFilter={faultFilter} setFaultFilter={setFaultFilter} setEditFault={setEditFault} setPage={setPage} currentUser={currentUser} />
            )}
            {page === "facility-form" && (
              <FacilityForm editFacility={editFacility} setEditFacility={setEditFacility} setPage={setPage} saveFacility={saveFacility} />
            )}
            {page === "facility-list" && (
              <FacilityList facilities={facilities} deleteFacility={deleteFacility} faults={faults} setEditFacility={setEditFacility} setPage={setPage} />
            )}
            {page === "account" && (
              <AccountManagement users={users} addUser={addUser} deleteUser={deleteUser} currentUser={currentUser} />
            )}
          </div>
        </div>
      </div>
